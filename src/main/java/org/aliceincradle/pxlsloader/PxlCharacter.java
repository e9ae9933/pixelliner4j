...
public class PxlCharacter {
	...
	private void load(NoelByteBuffer b, Settings s) {
		...
		while (b.size()>0) {
			...
			if (op.equals("%IMGD_SECTION%"))
				throw new IllegalArgumentException("PixelLiner error: does not support for " +
					                                   "compressed image data (not bug)");
	...
	private void writePackSection(NoelByteBuffer buf){
		...
		List<PixelLinerKey> keys=this.pxlPoses.stream()
			...
			.flatMap(frame->Arrays.stream(frame.layers))
			.filter(layer->!layer.isGroup())
			.map(layer->layer.id)
			.distinct().toList();
		int n=keys.size();
		List<BufferedImage> images=keys.stream().map(key->this.getImageByKey(key)).toList();
		List<Vector2i> sizes=images.stream().map(image->new Vector2i(image.getWidth(),image.getHeight())).toList();
		List<Vector2i> positions;
		int imageSize=128;
		while(true) {
			Vector2i[] v2i=Algorithm.packRectangles(sizes.toArray(Vector2i[]::new),
			                                               imageSize,
			                                     imageSize);
			if(v2i!=null) {
				positions=Arrays.asList(v2i);
				break;
			}
			imageSize*=2;
		}
		...
		for(int i=0;i<n;i++){
			...
			packed.putInt(positions.get(i).x);
			packed.putInt(positions.get(i).y);
			packed.putInt(images.get(i).getWidth());
			packed.putInt(images.get(i).getHeight());
		}
		...
		for(int i=0;i<n;i++){
			g.drawImage(images.get(i),positions.get(i).x,positions.get(i).y,null);
		}
		try{
			ByteArrayOutputStream baos=new ByteArrayOutputStream();
			ImageIO.write(img,"png",baos);
			byte[] imageBytes=baos.toByteArray();
//			Files.write(Path.of("test.png"),imageBytes);
	...
	@NotNull
	BufferedImage getImageByKey(PixelLinerKey id){
		var opt=this.pxlImages.stream()
			.filter(image->id.equals(image.id))
			.findFirst();
		if(opt.isPresent()) return opt.get().I==null?opt.get().P:opt.get().I;
		for (PxlImageAtlas atlas : this.pxlImageAtlases) {
			var opt2=Arrays.stream(atlas.pos).filter(pos->id.equals(pos.id))
				.findFirst();
			if(opt2.isPresent()) {
